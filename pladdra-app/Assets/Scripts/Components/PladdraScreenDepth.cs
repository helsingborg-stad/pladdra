...
namespace Lean.Touch
{
    ...
    [System.Serializable]
    public struct PladdraScreenDepth
    {
        ...
        public PladdraScreenDepth(ConversionType newConversion, int newLayers = Physics.DefaultRaycastLayers, float newDistance = 0.0f)
        {
        ...
        public bool TryConvert(ref Vector3 position, Vector2 screenPoint, GameObject gameObject = null, Transform ignore = null)
        {
            ...
            if (camera != null)
            {
                switch (Conversion)
                {
                    ...
                    case ConversionType.ARPlane:
                        {
                            var armanager = default(ARRaycastManager);

                            if (Exists(gameObject, ref armanager) == true)
                            {
                                ...
                                if (armanager.Raycast(screenPoint, arHits, TrackableType.PlaneWithinPolygon))
                                {
                                    ...
                                    position = hitPose.position + new Vector3(0, 0.01f, 0);
                                    // Debug.DrawLine(touchPosition, hitPose.position, Color.red, 0.1f);
                                    // Debug.DrawLine(hitPose.position, hitPose.forward, Color.green, 0.1f);
                                    // Debug.DrawRay(touchPosition, hitPose.position, Color.red, 0.1f);
...
namespace Lean.Touch.Editor
{
    ...
    [CustomPropertyDrawer(typeof(PladdraScreenDepth))]
    public class PladdraScreenDepth_Drawer : PropertyDrawer
    {
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            var conversion = (PladdraScreenDepth.ConversionType)property.FindPropertyRelative("Conversion").enumValueIndex;
            var height = base.GetPropertyHeight(property, label);
            var step = height + 2;

            switch (conversion)
            {
                case PladdraScreenDepth.ConversionType.FixedDistance: height += step * 2; break;
                case PladdraScreenDepth.ConversionType.DepthIntercept: height += step * 2; break;
                case PladdraScreenDepth.ConversionType.PhysicsRaycast: height += step * 3; break;
                case PladdraScreenDepth.ConversionType.PlaneIntercept: height += step * 3; break;
                case PladdraScreenDepth.ConversionType.ARPlane: height += step * 3; break;
                case PladdraScreenDepth.ConversionType.PathClosest: height += step * 3; break;
                case PladdraScreenDepth.ConversionType.AutoDistance: height += step * 2; break;
                case PladdraScreenDepth.ConversionType.HeightIntercept: height += step * 2; break;
            }
        ...
        public override void OnGUI(Rect rect, SerializedProperty property, GUIContent label)
        {
            ...
            EditorGUI.indentLevel++;
            {
                ...
                switch (conversion)
                {
                    case PladdraScreenDepth.ConversionType.FixedDistance:
                        {
                    ...
                    case PladdraScreenDepth.ConversionType.DepthIntercept:
                        {
                    ...
                    case PladdraScreenDepth.ConversionType.PhysicsRaycast:
                        {
                    ...
                    case PladdraScreenDepth.ConversionType.PlaneIntercept:
                        {
                        ...
                        break;
                    case PladdraScreenDepth.ConversionType.ARPlane:
                        {
                            DrawObjectProperty<ARRaycastManager>(ref rect, property, "Armanager", "The plane that will be intercepted.");
                            DrawProperty(ref rect, property, label, "Distance", "Offset", "The world space offset from the intercept hit point.");
                        }
                        break;

                    case PladdraScreenDepth.ConversionType.PathClosest:
                        {
                    ...
                    case PladdraScreenDepth.ConversionType.AutoDistance:
                        {
                            DrawProperty(ref rect, property, label, "Distance", "Offset", "The depth offset from the calculated point.");
                        }
                        break;

                    case PladdraScreenDepth.ConversionType.HeightIntercept:
                        {
