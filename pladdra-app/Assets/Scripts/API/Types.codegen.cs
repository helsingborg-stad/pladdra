using System;
using System.Collections.Generic;
using Newtonsoft.Json;
using GraphQL;

namespace Pladdra.API {
  public class Types {
    
    #region Asset
    public class Asset {
      #region members
      [JsonProperty("createdAt")]
      public string createdAt { get; set; }
    
      [JsonProperty("description")]
      public string description { get; set; }
    
      [JsonProperty("file")]
      public S3Object file { get; set; }
    
      [JsonProperty("fileFormat")]
      public AssetFileFormat fileFormat { get; set; }
    
      [JsonProperty("fileName")]
      public string fileName { get; set; }
    
      [JsonProperty("fileSize")]
      public int fileSize { get; set; }
    
      [JsonProperty("id")]
      public string id { get; set; }
    
      [JsonProperty("name")]
      public string name { get; set; }
    
      [JsonProperty("type")]
      public AssetType type { get; set; }
    
      [JsonProperty("updatedAt")]
      public string updatedAt { get; set; }
      #endregion
    }
    #endregion
    public enum AssetFileFormat {
      DWG,
      FBX,
      GLB,
      GLTF,
      OBJ
    }
    
    public enum AssetType {
      MESH
    }
    
    
    #region Block
    public class Block {
      #region members
      [JsonProperty("id")]
      public string id { get; set; }
    
      [JsonProperty("createdAt")]
      public string createdAt { get; set; }
    
      [JsonProperty("updatedAt")]
      public string updatedAt { get; set; }
    
      [JsonProperty("workspaceID")]
      public string workspaceID { get; set; }
    
      [JsonProperty("assetID")]
      public string assetID { get; set; }
    
      [JsonProperty("position")]
      public Vect3 position { get; set; }
    
      [JsonProperty("rotation")]
      public Quat rotation { get; set; }
    
      [JsonProperty("owner")]
      public string owner { get; set; }
      #endregion
    }
    #endregion
    
    #region CreateAssetInput
    public class CreateAssetInput {
      #region members
      public string description { get; set; }
    
      [JsonRequired]
      public S3ObjectInput file { get; set; }
    
      [JsonRequired]
      public AssetFileFormat fileFormat { get; set; }
    
      [JsonRequired]
      public string fileName { get; set; }
    
      [JsonRequired]
      public int fileSize { get; set; }
    
      public string id { get; set; }
    
      [JsonRequired]
      public string name { get; set; }
    
      [JsonRequired]
      public AssetType type { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region CreateBlockInput
    public class CreateBlockInput {
      #region members
      public string id { get; set; }
    
      public string createdAt { get; set; }
    
      public string updatedAt { get; set; }
    
      [JsonRequired]
      public string workspaceID { get; set; }
    
      [JsonRequired]
      public string assetID { get; set; }
    
      [JsonRequired]
      public Vect3Input position { get; set; }
    
      [JsonRequired]
      public QuatInput rotation { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region CreateWorkspaceInput
    public class CreateWorkspaceInput {
      #region members
      public string id { get; set; }
    
      [JsonRequired]
      public string name { get; set; }
    
      public string createdAt { get; set; }
    
      public string updatedAt { get; set; }
    
      public string description { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region DeleteAssetInput
    public class DeleteAssetInput {
      #region members
      [JsonRequired]
      public string id { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region DeleteBlockInput
    public class DeleteBlockInput {
      #region members
      [JsonRequired]
      public string id { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region DeleteWorkspaceInput
    public class DeleteWorkspaceInput {
      #region members
      [JsonRequired]
      public string id { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region ModelAssetByFileFormatCompositeKeyConditionInput
    public class ModelAssetByFileFormatCompositeKeyConditionInput {
      #region members
      public ModelAssetByFileFormatCompositeKeyInput beginsWith { get; set; }
    
      public List<ModelAssetByFileFormatCompositeKeyInput> between { get; set; }
    
      public ModelAssetByFileFormatCompositeKeyInput eq { get; set; }
    
      public ModelAssetByFileFormatCompositeKeyInput ge { get; set; }
    
      public ModelAssetByFileFormatCompositeKeyInput gt { get; set; }
    
      public ModelAssetByFileFormatCompositeKeyInput le { get; set; }
    
      public ModelAssetByFileFormatCompositeKeyInput lt { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region ModelAssetByFileFormatCompositeKeyInput
    public class ModelAssetByFileFormatCompositeKeyInput {
      #region members
      public string createdAt { get; set; }
    
      public string updatedAt { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region ModelAssetConditionInput
    public class ModelAssetConditionInput {
      #region members
      public List<ModelAssetConditionInput> and { get; set; }
    
      public ModelStringInput createdAt { get; set; }
    
      public ModelStringInput description { get; set; }
    
      public ModelAssetFileFormatInput fileFormat { get; set; }
    
      public ModelStringInput fileName { get; set; }
    
      public ModelIntInput fileSize { get; set; }
    
      public ModelStringInput name { get; set; }
    
      public ModelAssetConditionInput not { get; set; }
    
      public List<ModelAssetConditionInput> or { get; set; }
    
      public ModelAssetTypeInput type { get; set; }
    
      public ModelStringInput updatedAt { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region ModelAssetConnection
    public class ModelAssetConnection {
      #region members
      [JsonProperty("items")]
      public List<Asset> items { get; set; }
    
      [JsonProperty("nextToken")]
      public string nextToken { get; set; }
      #endregion
    }
    #endregion
    
    #region ModelAssetFileFormatInput
    public class ModelAssetFileFormatInput {
      #region members
      public AssetFileFormat? eq { get; set; }
    
      public AssetFileFormat? ne { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region ModelAssetFilterInput
    public class ModelAssetFilterInput {
      #region members
      public List<ModelAssetFilterInput> and { get; set; }
    
      public ModelStringInput createdAt { get; set; }
    
      public ModelStringInput description { get; set; }
    
      public ModelAssetFileFormatInput fileFormat { get; set; }
    
      public ModelStringInput fileName { get; set; }
    
      public ModelIntInput fileSize { get; set; }
    
      public ModelIdInput id { get; set; }
    
      public ModelStringInput name { get; set; }
    
      public ModelAssetFilterInput not { get; set; }
    
      public List<ModelAssetFilterInput> or { get; set; }
    
      public ModelAssetTypeInput type { get; set; }
    
      public ModelStringInput updatedAt { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region ModelAssetTypeInput
    public class ModelAssetTypeInput {
      #region members
      public AssetType? eq { get; set; }
    
      public AssetType? ne { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    public enum ModelAttributeTypes {
      _null,
      binary,
      binarySet,
      @bool,
      list,
      map,
      number,
      numberSet,
      @string,
      stringSet
    }
    
    
    #region ModelBlockBlocksByWorkspaceCompositeKeyConditionInput
    public class ModelBlockBlocksByWorkspaceCompositeKeyConditionInput {
      #region members
      public ModelBlockBlocksByWorkspaceCompositeKeyInput eq { get; set; }
    
      public ModelBlockBlocksByWorkspaceCompositeKeyInput le { get; set; }
    
      public ModelBlockBlocksByWorkspaceCompositeKeyInput lt { get; set; }
    
      public ModelBlockBlocksByWorkspaceCompositeKeyInput ge { get; set; }
    
      public ModelBlockBlocksByWorkspaceCompositeKeyInput gt { get; set; }
    
      public List<ModelBlockBlocksByWorkspaceCompositeKeyInput> between { get; set; }
    
      public ModelBlockBlocksByWorkspaceCompositeKeyInput beginsWith { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region ModelBlockBlocksByWorkspaceCompositeKeyInput
    public class ModelBlockBlocksByWorkspaceCompositeKeyInput {
      #region members
      public string updatedAt { get; set; }
    
      public string createdAt { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region ModelBlockConditionInput
    public class ModelBlockConditionInput {
      #region members
      public ModelStringInput createdAt { get; set; }
    
      public ModelStringInput updatedAt { get; set; }
    
      public ModelIdInput workspaceID { get; set; }
    
      public ModelIdInput assetID { get; set; }
    
      public List<ModelBlockConditionInput> and { get; set; }
    
      public List<ModelBlockConditionInput> or { get; set; }
    
      public ModelBlockConditionInput not { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region ModelBlockConnection
    public class ModelBlockConnection {
      #region members
      [JsonProperty("items")]
      public List<Block> items { get; set; }
    
      [JsonProperty("nextToken")]
      public string nextToken { get; set; }
      #endregion
    }
    #endregion
    
    #region ModelBlockFilterInput
    public class ModelBlockFilterInput {
      #region members
      public ModelIdInput id { get; set; }
    
      public ModelStringInput createdAt { get; set; }
    
      public ModelStringInput updatedAt { get; set; }
    
      public ModelIdInput workspaceID { get; set; }
    
      public ModelIdInput assetID { get; set; }
    
      public List<ModelBlockFilterInput> and { get; set; }
    
      public List<ModelBlockFilterInput> or { get; set; }
    
      public ModelBlockFilterInput not { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region ModelBooleanInput
    public class ModelBooleanInput {
      #region members
      public bool? attributeExists { get; set; }
    
      public ModelAttributeTypes? attributeType { get; set; }
    
      public bool? eq { get; set; }
    
      public bool? ne { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region ModelFloatInput
    public class ModelFloatInput {
      #region members
      public bool? attributeExists { get; set; }
    
      public ModelAttributeTypes? attributeType { get; set; }
    
      public List<double?> between { get; set; }
    
      public double? eq { get; set; }
    
      public double? ge { get; set; }
    
      public double? gt { get; set; }
    
      public double? le { get; set; }
    
      public double? lt { get; set; }
    
      public double? ne { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region ModelIdInput
    public class ModelIdInput {
      #region members
      public bool? attributeExists { get; set; }
    
      public ModelAttributeTypes? attributeType { get; set; }
    
      public string beginsWith { get; set; }
    
      public List<string> between { get; set; }
    
      public string contains { get; set; }
    
      public string eq { get; set; }
    
      public string ge { get; set; }
    
      public string gt { get; set; }
    
      public string le { get; set; }
    
      public string lt { get; set; }
    
      public string ne { get; set; }
    
      public string notContains { get; set; }
    
      public ModelSizeInput size { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region ModelIntInput
    public class ModelIntInput {
      #region members
      public bool? attributeExists { get; set; }
    
      public ModelAttributeTypes? attributeType { get; set; }
    
      public List<int?> between { get; set; }
    
      public int? eq { get; set; }
    
      public int? ge { get; set; }
    
      public int? gt { get; set; }
    
      public int? le { get; set; }
    
      public int? lt { get; set; }
    
      public int? ne { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region ModelSizeInput
    public class ModelSizeInput {
      #region members
      public List<int?> between { get; set; }
    
      public int? eq { get; set; }
    
      public int? ge { get; set; }
    
      public int? gt { get; set; }
    
      public int? le { get; set; }
    
      public int? lt { get; set; }
    
      public int? ne { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    public enum ModelSortDirection {
      ASC,
      DESC
    }
    
    
    #region ModelStringInput
    public class ModelStringInput {
      #region members
      public bool? attributeExists { get; set; }
    
      public ModelAttributeTypes? attributeType { get; set; }
    
      public string beginsWith { get; set; }
    
      public List<string> between { get; set; }
    
      public string contains { get; set; }
    
      public string eq { get; set; }
    
      public string ge { get; set; }
    
      public string gt { get; set; }
    
      public string le { get; set; }
    
      public string lt { get; set; }
    
      public string ne { get; set; }
    
      public string notContains { get; set; }
    
      public ModelSizeInput size { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region ModelWorkspaceConditionInput
    public class ModelWorkspaceConditionInput {
      #region members
      public ModelStringInput name { get; set; }
    
      public ModelStringInput createdAt { get; set; }
    
      public ModelStringInput updatedAt { get; set; }
    
      public ModelStringInput description { get; set; }
    
      public List<ModelWorkspaceConditionInput> and { get; set; }
    
      public List<ModelWorkspaceConditionInput> or { get; set; }
    
      public ModelWorkspaceConditionInput not { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region ModelWorkspaceConnection
    public class ModelWorkspaceConnection {
      #region members
      [JsonProperty("items")]
      public List<Workspace> items { get; set; }
    
      [JsonProperty("nextToken")]
      public string nextToken { get; set; }
      #endregion
    }
    #endregion
    
    #region ModelWorkspaceFilterInput
    public class ModelWorkspaceFilterInput {
      #region members
      public ModelIdInput id { get; set; }
    
      public ModelStringInput name { get; set; }
    
      public ModelStringInput createdAt { get; set; }
    
      public ModelStringInput updatedAt { get; set; }
    
      public ModelStringInput description { get; set; }
    
      public List<ModelWorkspaceFilterInput> and { get; set; }
    
      public List<ModelWorkspaceFilterInput> or { get; set; }
    
      public ModelWorkspaceFilterInput not { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region Mutation
    public class Mutation {
      #region members
      [JsonProperty("createAsset")]
      public Asset createAsset { get; set; }
    
      [JsonProperty("createBlock")]
      public Block createBlock { get; set; }
    
      [JsonProperty("createWorkspace")]
      public Workspace createWorkspace { get; set; }
    
      [JsonProperty("deleteAsset")]
      public Asset deleteAsset { get; set; }
    
      [JsonProperty("deleteBlock")]
      public Block deleteBlock { get; set; }
    
      [JsonProperty("deleteWorkspace")]
      public Workspace deleteWorkspace { get; set; }
    
      [JsonProperty("updateAsset")]
      public Asset updateAsset { get; set; }
    
      [JsonProperty("updateBlock")]
      public Block updateBlock { get; set; }
    
      [JsonProperty("updateWorkspace")]
      public Workspace updateWorkspace { get; set; }
      #endregion
    }
    #endregion
    
    #region Quat
    public class Quat {
      #region members
      [JsonProperty("x")]
      public int x { get; set; }
    
      [JsonProperty("y")]
      public int y { get; set; }
    
      [JsonProperty("z")]
      public int z { get; set; }
    
      [JsonProperty("w")]
      public int w { get; set; }
      #endregion
    }
    #endregion
    
    #region QuatInput
    public class QuatInput {
      #region members
      [JsonRequired]
      public int x { get; set; }
    
      [JsonRequired]
      public int y { get; set; }
    
      [JsonRequired]
      public int z { get; set; }
    
      [JsonRequired]
      public int w { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region Query
    public class Query {
      #region members
      [JsonProperty("assetsByFileFormat")]
      public ModelAssetConnection assetsByFileFormat { get; set; }
    
      [JsonProperty("getAsset")]
      public Asset getAsset { get; set; }
    
      [JsonProperty("getBlock")]
      public Block getBlock { get; set; }
    
      [JsonProperty("getWorkspace")]
      public Workspace getWorkspace { get; set; }
    
      [JsonProperty("listAssets")]
      public ModelAssetConnection listAssets { get; set; }
    
      [JsonProperty("listBlocks")]
      public ModelBlockConnection listBlocks { get; set; }
    
      [JsonProperty("listWorkspaces")]
      public ModelWorkspaceConnection listWorkspaces { get; set; }
      #endregion
    }
    #endregion
    
    #region S3Object
    public class S3Object {
      #region members
      [JsonProperty("bucket")]
      public string bucket { get; set; }
    
      [JsonProperty("key")]
      public string key { get; set; }
    
      [JsonProperty("region")]
      public string region { get; set; }
      #endregion
    }
    #endregion
    
    #region S3ObjectInput
    public class S3ObjectInput {
      #region members
      public string _url { get; set; }
    
      [JsonRequired]
      public string bucket { get; set; }
    
      [JsonRequired]
      public string key { get; set; }
    
      public string localUri { get; set; }
    
      public string mimeType { get; set; }
    
      [JsonRequired]
      public string region { get; set; }
    
      public string url { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region Subscription
    public class Subscription {
      #region members
      [JsonProperty("onCreateAsset")]
      public Asset onCreateAsset { get; set; }
    
      [JsonProperty("onCreateBlock")]
      public Block onCreateBlock { get; set; }
    
      [JsonProperty("onCreateWorkspace")]
      public Workspace onCreateWorkspace { get; set; }
    
      [JsonProperty("onDeleteAsset")]
      public Asset onDeleteAsset { get; set; }
    
      [JsonProperty("onDeleteBlock")]
      public Block onDeleteBlock { get; set; }
    
      [JsonProperty("onDeleteWorkspace")]
      public Workspace onDeleteWorkspace { get; set; }
    
      [JsonProperty("onUpdateAsset")]
      public Asset onUpdateAsset { get; set; }
    
      [JsonProperty("onUpdateBlock")]
      public Block onUpdateBlock { get; set; }
    
      [JsonProperty("onUpdateWorkspace")]
      public Workspace onUpdateWorkspace { get; set; }
      #endregion
    }
    #endregion
    
    #region UpdateAssetInput
    public class UpdateAssetInput {
      #region members
      public string createdAt { get; set; }
    
      public string description { get; set; }
    
      public S3ObjectInput file { get; set; }
    
      public AssetFileFormat? fileFormat { get; set; }
    
      public string fileName { get; set; }
    
      public int? fileSize { get; set; }
    
      [JsonRequired]
      public string id { get; set; }
    
      public string name { get; set; }
    
      public AssetType? type { get; set; }
    
      public string updatedAt { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region UpdateBlockInput
    public class UpdateBlockInput {
      #region members
      [JsonRequired]
      public string id { get; set; }
    
      public string createdAt { get; set; }
    
      public string updatedAt { get; set; }
    
      public string workspaceID { get; set; }
    
      public string assetID { get; set; }
    
      public Vect3Input position { get; set; }
    
      public QuatInput rotation { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region UpdateWorkspaceInput
    public class UpdateWorkspaceInput {
      #region members
      [JsonRequired]
      public string id { get; set; }
    
      public string name { get; set; }
    
      public string createdAt { get; set; }
    
      public string updatedAt { get; set; }
    
      public string description { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region Vect3
    public class Vect3 {
      #region members
      [JsonProperty("x")]
      public int x { get; set; }
    
      [JsonProperty("y")]
      public int y { get; set; }
    
      [JsonProperty("z")]
      public int z { get; set; }
      #endregion
    }
    #endregion
    
    #region Vect3Input
    public class Vect3Input {
      #region members
      [JsonRequired]
      public int x { get; set; }
    
      [JsonRequired]
      public int y { get; set; }
    
      [JsonRequired]
      public int z { get; set; }
      #endregion
    
      #region methods
      public dynamic GetInputObject()
      {
        IDictionary<string, object> d = new System.Dynamic.ExpandoObject();
    
        var properties = GetType().GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
        foreach (var propertyInfo in properties)
        {
          var value = propertyInfo.GetValue(this);
          var defaultValue = propertyInfo.PropertyType.IsValueType ? Activator.CreateInstance(propertyInfo.PropertyType) : null;
    
          var requiredProp = propertyInfo.GetCustomAttributes(typeof(JsonRequiredAttribute), false).Length > 0;
          if (requiredProp || value != defaultValue)
          {
            d[propertyInfo.Name] = value;
          }
        }
        return d;
      }
      #endregion
    }
    #endregion
    
    #region Workspace
    public class Workspace {
      #region members
      [JsonProperty("id")]
      public string id { get; set; }
    
      [JsonProperty("name")]
      public string name { get; set; }
    
      [JsonProperty("createdAt")]
      public string createdAt { get; set; }
    
      [JsonProperty("updatedAt")]
      public string updatedAt { get; set; }
    
      [JsonProperty("description")]
      public string description { get; set; }
    
      [JsonProperty("blocks")]
      public ModelBlockConnection blocks { get; set; }
    
      [JsonProperty("owner")]
      public string owner { get; set; }
      #endregion
    }
    #endregion
  }
  
}
